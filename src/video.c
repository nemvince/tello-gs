...
static void *video_thread(void *arg)
{
    ...
    fprintf(stderr, "[video] listening on port %d\n", VIDEO_PORT);
    ...
    bool first_pkt = true, first_frame = true;
    ...
    while (g_running)
    {
        ...
        if (first_pkt)
        {
            fprintf(stderr, "[video] first UDP packet (%zd bytes)\n", n);
            first_pkt = false;
        }
        pkt_count++;
        ...
        while (data_size > 0)
        {
            ...
            while (avcodec_receive_frame(ctx, frame) == 0)
            {
                ...
                if (first_frame)
                {
                    fprintf(stderr, "[video] first decoded frame %dx%d (after %lu UDP pkts)\n", w, h, pkt_count);
                    first_frame = false;
                }
